...
test('loadContent should load Panel items when preference is true', async () => {
...
test('loadContent should preserve existing state properties', async () => {
  ...
  const state: PanelState & { disposed?: boolean } = {
    ...createDefaultState(),
    disposed: true,
    uid: 4,
  }
...
test('loadContent should handle multiple Panel items', async () => {
  using mockRendererRpc = RendererWorker.registerMockRpc({
    'ExtensionHostManagement.activateByEvent': async () => {},
