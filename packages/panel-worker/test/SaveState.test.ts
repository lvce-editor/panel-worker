...
test('saveState should return empty arrays when state has no items', () => {
  const uid = 1
  const state: PanelState = { ...createDefaultState(), uid }
...
test('saveState should work with different uid values', () => {
  const uid1 = 10
  const state1: PanelState = { ...createDefaultState(), uid: uid1 }
  PanelStates.set(uid1, state1, state1)
  ...
  const state2: PanelState = {
    ...createDefaultState(),
    statusBarItemsLeft: [item],
    uid: uid2,
  }
  PanelStates.set(uid2, state2, state2)
