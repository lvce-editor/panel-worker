...
test('renderItems should return SetDom2 command with uid and empty dom when no items', () => {
  const oldState: PanelState = createDefaultState()
  const newState: PanelState = { ...createDefaultState(), initial: false, uid: 1 }
...
test('renderItems should use uid from newState', () => {
  const oldState: PanelState = { ...createDefaultState(), uid: 10 }
  const newState: PanelState = { ...createDefaultState(), uid: 20 }
